resource "azurerm_virtual_network" "ResVnet-eastus" {
...
resource "azurerm_subnet" "ResSubnet-eastus" {
  count                = length(var.resource_subnet_name_vnet_usa01)
  name                 = var.resource_subnet_name_vnet_usa01[count.index]
  resource_group_name  = azurerm_resource_group.ResGroup.name
  virtual_network_name = azurerm_virtual_network.ResVnet-eastus.name
